...
type roomFindOutput struct {
	...
	PartialFailures []map[string]interface{} `json:"partial_failures,omitempty"`
}
...
func collectRoomFindResults(slots []roomFindSlot, limit int, fetch func(roomFindSlot) ([]*roomFindSuggestion, error)) (*roomFindOutput, error) {
	...
	out := &roomFindOutput{
		...
		PartialFailures: []map[string]interface{}{},
	}
	...
	for _, slot := range slots {
		...
		go func() {
			...
			if err != nil {
				...
				out.PartialFailures = append(out.PartialFailures, map[string]interface{}{
					"start": slot.Start,
					"end":   slot.End,
					"error": err.Error(),
				})
	...
	if len(out.TimeSlots) == 0 && firstErr != nil {
		return out, firstErr
	}
...
var CalendarRoomFind = common.Shortcut{
	...
	Flags: []common.Flag{
		...
		{Name: flagFloor, Type: "string", Desc: "meeting room floor or area locator constraint (recommended canonical form such as F2; floor/area locators such as 2L, 2F, or 2F-A should go here)"},
		{Name: flagRoomName, Type: "string", Desc: "meeting room name constraint (for room names such as 木星 or 02, not floor-like locators)"},
	...
	Execute: func(ctx context.Context, runtime *common.RuntimeContext) error {
		...
		runtime.OutFormat(out, &output.Meta{Count: len(out.TimeSlots)}, func(w io.Writer) {
			...
			if len(out.PartialFailures) > 0 {
				fmt.Fprintf(w, "%d partial failure(s)\n", len(out.PartialFailures))
			}
		})
		return nil
	},
