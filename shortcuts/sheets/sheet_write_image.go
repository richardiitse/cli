...
var SheetWriteImage = common.Shortcut{
	...
	Flags: []common.Flag{
		...
		{Name: "range", Desc: "target cell (e.g. A1 or <sheetId>!A1). Start and end cell must be the same"},
	...
	Execute: func(ctx context.Context, runtime *common.RuntimeContext) error {
		...
		// Resolve the target cell range.
		pointRange := runtime.Str("range")
		if pointRange == "" && runtime.Str("sheet-id") != "" {
			pointRange = runtime.Str("sheet-id")
		}
		if pointRange == "" {
			sheetID, err := getFirstSheetID(runtime, token)
			if err != nil {
				return err
			}
			pointRange = sheetID
		}
		pointRange = normalizePointRange(runtime.Str("sheet-id"), pointRange)
