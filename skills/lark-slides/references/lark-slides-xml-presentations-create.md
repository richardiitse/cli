...
> 协议层仍然允许 `<presentation>` 包含 `<slide>`，但当前 `xml_presentations.create` 命令不建议承载 slide 内容；实际页面请改用 `xml_presentation.sildes.create`。
...
| 400 | create 内容超出支持范围 | 只传标题和长宽，slide 内容改用 `xml_presentation.sildes.create` |
