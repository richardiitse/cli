...
```bash
# 先读取 XML 内容，确认待删除页面
lark-cli slides xml_presentations get --params '{"xml_presentation_id":"slides_example_presentation_id"}' | jq -r '.xml_presentation.content'

# 然后按已知 slide_id 删除
lark-cli slides xml_presentation.slides delete --params '{"xml_presentation_id":"slides_example_presentation_id","slide_id":"slide_example_id"}'
```
...
```bash
lark-cli slides xml_presentation.slides create --params '{"xml_presentation_id":"slides_example_presentation_id"}' --data '{
  "slide": {
...
for slide_id in sld_a sld_b sld_c; do
  lark-cli slides xml_presentation.slides delete --params "{\"xml_presentation_id\":\"slides_example_presentation_id\",\"slide_id\":\"$slide_id\"}"
done
```
